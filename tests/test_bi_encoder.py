...
@pytest.fixture(scope="module", params=[TestModel, ColBERTModel, XTRModel])
def model(
    model_name_or_path: str, request: SubRequest
) -> TestModel | ColBERTModel | XTRModel:
    Model = request.param
...
def module(
    model: TestModel | ColBERTModel | XTRModel, request: SubRequest
) -> TestModule | ColBERTModule | XTRModule:
...
def test_doc_padding(
    relevance_run_datamodule: LightningIRDataModule,
    model: TestModel | ColBERTModel | XTRModel,
):
    if not isinstance(relevance_run_datamodule.config, BiEncoderConfig):
        pytest.skip()
...
def test_training_step(
    module: TestModule | ColBERTModule | XTRModule,
    tuples_datamodule: LightningIRDataModule,
):
    if not isinstance(tuples_datamodule.config, BiEncoderConfig):
        pytest.skip()
