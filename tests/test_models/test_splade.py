...
def test_same_as_splade():
    ...
    orig_query_encoding = orig_model.transformer_rep.tokenizer(
        query, return_tensors="pt"
    )
    orig_doc_encodings = orig_model.transformer_rep.tokenizer(
    ...
    tokenizer = SpladeModel.config_class.tokenizer_class.from_pretrained(
    ...
    assert torch.allclose(
        query_embeddings.embeddings.squeeze(1), orig_query_embeddings, atol=1e-6
    )
    assert torch.allclose(
        doc_embeddings.embeddings.squeeze(1), orig_doc_embeddings, atol=1e-5
    )
