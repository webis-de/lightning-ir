...
LightningIR heavily relies on [PyTorch Lightning's](https://lightning.ai/docs/pytorch/stable/) core API. We refer to Pytorch Lightning's excellent [documentation](https://lightning.ai/docs/pytorch/stable/) for most of the core functionality. The following sections provide an overview of the additionally functionality added by LightningIR.

### Trainer

The `Trainer` is the entrypoint for fine-tuning and running neural ranking models. It provides four main methods:

- `fit`: Fine-tunes a neural ranking model.
- `index`: Indexes a collection of documents.
- `search`: Searches for relevant documents.
- `re_rank`: Re-ranks a set of retrieved documents.

Each method requires a [LightningIRModule](#module) and a [LightningIRDataModule](#data) object. The `index` method addtionally requires an `IndexCallback` to handle writing the computed embeddings to an index. The `search` and `re_rank` methods addtionally require a `RankCallback` to handle writing the computed scores to a TREC-style run file.

### Model

A `LightningIRModel` is a backbone-agnostic wrapper around a pre-trained model Transformer-based language model from Hugging Face's [transformers](https://huggingface.co/transformers/) library. It uses a backbone model to generate embeddings from on a tokenized input sequence and further processes these embeddings based on parameters specified in a `LightningIRConfig`. Two types of `LightningIRModel`s exist: `BiEncoderModel`s and `CrossEncoderModel`s. Depending on the type of model, different methods are used to generate scores for a query-document pair.

To create a new `LightningIRModel`, a backbone `transformers` model must be combined with a `LightningIRConfig` using the `LightningIRModelClassFactory`. For example, to create a `BiEncoderModel` with a BERT backbone, the following code can be used:

```python
from transformers import BertModel
from lightning_ir import LightningIRModelClassFactory, BiEncoderConfig

BertBiEncoder = LightningIRModelClassFactory(BertModel, BiEncoderConfig)
model = BertBiEncoder.from_pretrained("bert-base-uncased") # has type `BertBiEncoderModel`
```

Pre-finetuned `LightningIR` models are automagically registered with the `transformers` library and can be loaded using the `from_pretrained` method. For example, to load a pre-finetuned `monoelectra-base` model, the following code can be used:

```python
import lightning_ir # Register the model with transformers
from transformers import AutoModel

model = AutoModel.from_pretrained("webis/monoelectra-base") # has type `CrossEncoderElectraModel`
```

#### Bi-encoder

A [`BiEncoderModel`](lightning_ir/bi_encoder/model.py) processes queries and documents separately and computes scores based on the similarity of the query and document embeddings. A [`BiEncoderConfig`](lightning_ir/bi_encoder/config.py) specifies how the embeddings are processed and optionally aggregated. The config is kept as generic as possible to support a wide range of bi-encoders. The configuration options are also asymmetric, meaning that different configurations can be used for queries and documents. See the [`BiEncoderConfig`](lightning_ir/bi_encoder/config.py) class for more details.
...
A [`CrossEncoderModel`](lightning_ir/cross_encoder/model.py) processes queries and documents together and computes a score based on the contextualized embeddings of the query and document. A [`CrossEncoderConfig`](lightning_ir/cross_encoder/config.py) specifies how the embeddings are processed and aggregated to output a similarity score for each document. The config is kept as generic as possible to support a wide range of cross-encoders. See the [`CrossEncoderConfig`](lightning_ir/cross_encoder/config.py) class for more details.

### Module

A `LightningIRModule` is a PyTorch Lightning module that wraps a `LightningIRModel` and provides the necessary methods to fine-tune and run the model. The module is responsible for loading the model, processing the input data, and computing the loss and scores.

### Data

Lightning IR is tightly integrated with [`ir-datasets`](https://github.com/allenai/ir_datasets/) to provide easy access to various information retrieval collections and datasets.
